...
import { timeSinceText } from "@/app/components/tools/TimeSince";
...
export async function generateMetadata({ params }: { params: { tag: string } }): Promise<Metadata> {
  ...
  if (data.length > 0) {
    ...
    return {
      title: `${article.tag} - Artikler og Indsigter`,
      description: Array.isArray(article.teaser) ? article.teaser.join(",") : article.teaser,
      keywords: `Tag ${article.tag} - Artikler og Indsigter`,
      openGraph: {
      title: `${article.tag}`,
      description: `${article.teaser},`,
      url: `https://xn--pengehjrnet-mgb.dk/artikler/tag/${article.tagSlug}`,
      type: 'website',
      siteName: 'Pengehjørnet',
      locale: 'da_DK',
      images: [
        {
        url: article.image ? urlFor(article.image).format("webp")
      .width(400)
      .height(300)
      .fit("fill")
      .quality(85)
      ...
      title: `${article.tag} - Artikler og Indsigter`,
      description: `${article.teaser}`,
      images: article.image ? urlFor(article.image).format("webp")
      .width(400)
      .height(300)
      .fit("fill")
      .quality(85)
  ...
  <ol className="inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div className="flex items-center">
        ...
        <Link href="/sider/referencer/tag" className="text-fade_color_light dark:text-fade_color_dark hover:text-gray-900 dark:hover:text-gray-400 ml-1 md:ml-2 text-sm font-medium ">Tags</Link>
      </div>
    </li>
    <li className=" cursor-default " aria-current="page">
      <div className="flex items-center">
        <svg className="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"></path></svg>
        <span className="text-accent_color_light dark:text-accent_color_dark ml-1 md:ml-2 text-sm font-medium capitalize "> { params.tag} </span>
      </div>
    </li>
  </ol>
</nav>
...
<section className=" grid md:grid-cols-[auto_1fr] mx-auto ">
      <div className="container px-3 lg:px-6  pt-0 max-w-[1000px]">
         <div>
          ...
          <div className="inline-block md:hidden">
            <TrendingArticlesList dayInterval={30} startIndex={0} endIndex={5} tag={data[0].tagSlug} />
      ...
      <div className="hidden xl:inline-block"><TrendingArticlesList dayInterval={30} startIndex={0} endIndex={5} tag={data[0].tagSlug} /></div>
</section>
</>
  );
}

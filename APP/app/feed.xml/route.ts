...
export async function GET() {
  ...
  allData.forEach((article) => {
    ...
    if (article.overview) {
      filteredOverview = article.overview.filter(
        (block) =>
    ...
    const imageSize = article.imageTags.size
      ? article.imageTags.size.toString()
      : "0";
    const imageExtension = article.imageTags.extension
      ? article.imageTags.extension
      : "jpeg";
    const articleDescription =
      portableTextToHtml(filteredOverview) || article.description;
    ...
    feed.item({
      ...
      url:
        article._type === "msnScrollFeed"
          ? `${theme.site_url}/guide/${article.articleSlug}`
          : `${theme.site_url}/artikel/${article.articleSlug}`,
      guid: article._id,
